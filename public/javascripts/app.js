$(function() {
	...
	let FormBehaviorEngine = {
		...
		bindEvents: function() {
			...
			this.$form.on('click', '.cancel', this.cancel.bind(this));
		...
		cancel: function(e) {
			e.preventDefault();
			AllContactsEngine.renderMainPage();
			this.$form.closest('.new').slideUp(400, function() {
				this.remove();
			});
	...
	let AllContactsEngine = {
		...
		renderMainPage: function() {
			...
			this.getAllContacts().then(function(data) {
				...
				setTimeout(self.bindEvents.bind(self), 2000);
			});
		},
		...
		bindEvents: function() {
			$('main').on('click','input[name="edit"]', this.editContact.bind(this));
			...
			$('input[name="search"]').on('keyup click', this.filterByName.bind(this));
		},
		...
		filterByTag: function(e) {
			e.stopPropagation();
			let text = $(e.target).text().replace('#', '');
			let regex = new RegExp(text);

			this.filter(text, regex, 'tags');
		},
		...
		filterByName: function(e) {
			...
			this.filter(text, regex, 'full_name');
		},

		filter: function(string, regexString, type) {
			let matches;

			this.getAllContacts().then(function(result) {
				matches = result.filter(function(contact) {
					return regexString.test(contact[type]);
				})

				$('main').find('div.contact:visible').hide();

				matches.forEach(function(contact) {
					let id = contact.id;
					$('.contact[data-id=' + id + ']').show();
				});
			});
		},
